...
def main():
    ...
    traces_hdf_path = Path(prep_cfg.output_directory) / args.dataset / "traces_data.h5"
    traces_data = load_traces_data_to_dict(traces_hdf_path)

    if len(args.ibis_path) == 0:
        ibis_path = Path.cwd() / f"{args.dataset}_ibis_combined.h5"
    else:
        ibis_path = Path(args.ibis_path)
