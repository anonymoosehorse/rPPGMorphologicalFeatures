...
from .custom_callbacks import PerSampleTestLogger


def config_exists_in_project(cfg):
    cfgs_in_project = list((Path("model_checkpoints") / cfg.comet.project).rglob("*.yaml"))
...
def initialize_callbacks(cfg,checkpoint_dir):
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        monitor=cfg.train.checkpoint.monitor,  # Metric to monitor for best performance
    ...
    per_sample_logger = PerSampleTestLogger(cfg, save_path=checkpoint_dir/"TestResults.csv")
    training_callbacks.append(per_sample_logger)
...
def run_training(checkpoint_dir,cfg,data_cfg,experiment_name=None,debug_mode=False):
    ...
    if not debug_mode:
        ...
        if experiment_name is None:
            experiment_name = cfg.comet.project + "/" + comet_logger.experiment.name
    ...
    data_path = get_data_path(
        cfg.dataset.root,cfg.dataset.name,cfg.model.input_representation,cfg.dataset.use_gt)    
    ...
    if val_loader is not None:        
        trainer.test(runner,ckpt_path=checkpoint_path, dataloaders=val_loader)
