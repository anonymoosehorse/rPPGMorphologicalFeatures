...
if __name__ == '__main__':
    ...
    x = torch.tensor(np.loadtxt("C:/Users/ruben/Documents/thesis/data/vipl/split_traces/p1_v1_source1_0.csv", delimiter=',')).type(torch.FloatTensor)
    x = torch.stack((x, x))
    ...
    for i in range(1000):
        output = model(x)
        print(output.item())
        loss = loss_fn(output, torch.Tensor([300]).to(torch.device('cuda:0')))
        loss.backward()
        optimizer.step()