...
#         v = file.split('/')[-1].split('_')[1]
#         s = file.split('/')[-1].split('_')[2].split('.')[0]
#         video_path = config.FACE_DATA_DIR + f"{p}/{v}/{s}/used_video.avi"
...
#         npy_path = f"{config.ST_MAPS_PATH}{filename[0]+'_v'+filename[1]+'_s'+filename[2]}.npy"
#         if os.path.exists(npy_path):
#             video_meta_data["num_maps"] = np.load(npy_path).shape[0]
#         else:
...
#         hr_df.to_csv(config.HOME_DIR + f"../data/hr_rhythmnet/{file_name}.csv", index=False)
#         # print("eheee")
...
# def compute_hr_for_cwtnet():
#     data_files = glob.glob(config.SPLIT_SIGNAL_DIR + "*.csv")
#     # for file in tqdm(data_files):
#     print(len(data_files))
...
#         bpm = [round(duration / 5000.0 * 60.0, 2)]
#         np.savetxt(config.CWT_TARGET_PATH + fname, bpm, delimiter=',')



if __name__ == '__main__':
    compute_hr_for_cwtnet()
