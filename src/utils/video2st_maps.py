...
def get_spatio_temporal_map_threaded(file):
    ...
    #np_path = save_path+file_name+" .npy"
    np_path = config.ST_MAPS_PATH + p_name + "_v" + v_name + "_s" + s_name +".npy"
    np.save(np_path, maps)

    #img_path = config.ST_MAPS_PATH + p_name + "_" + folder_name+".jpg"
    #cv2.imwrite(img_path, maps[math.floor(len(maps)/2)])

    hr_path = config.ST_MAPS_PATH + "HR_" + p_name + "_v" + v_name + "_s" + s_name +".csv"
    hr_data = "/".join(file.split('/')[:-1]) + "/gt_HR.csv"
    copy(hr_data, hr_path)
    return 1
...
def get_spatio_temporal_map_threaded_wrapper():
    video_files = glob.glob(config.FACE_DATA_DIR + '**/used_video.avi', recursive=True)
    # video_files.reverse()
    # print(video_files)
    ...
    with parallel_backend("loky", inner_max_num_threads=4):
        Parallel(n_jobs=32)(delayed(get_spatio_temporal_map_threaded)(file) for file in tqdm(video_files))

    for file in video_files:
        file = file.replace(os.sep, '/')
        get_spatio_temporal_map_threaded(file)
    end = time.time()
...
def get_spatio_temporal_map():
    ...
    # video_files = ['/home/rsangers/data.avi']
...
def preprocess_video_to_st_maps(video_path, output_shape, clip_size):
    ...
    #detector = get_haarcascade(config.haarcascade_url)
    eye_detector = get_eye_cascade()

    detector = get_haarcascade("https://raw.githubusercontent.com/opencv/opencv/master/data/haarcascades/haarcascade_frontalface_default.xml")
    ...
    print("Found face in "+str(nFound/len(frames))+" of the frames")
...
def rename_source3_files():
    video_files = glob.glob(config.FACE_DATA_DIR + '**/source3/video.avi', recursive=True)
    for video in video_files:
        video_path = "/".join(video.split('/')[:-1])
        os.rename(video, os.path.join(video_path, "used_video.avi"))
    print("Successfully renamed "+str(len(video_files))+" files")


if __name__ == '__main__':
    # rename_source3_files()
    get_spatio_temporal_map_threaded_wrapper()

    #get_frames_and_video_meta_data('/media/rsangers/30BE-ADAC/p1/v1/source1')
    #get_spatio_temporal_map()
    #video_path = '/media/rsangers/30BE-ADAC/p1/v1/source1/video.avi'
    #output_shape = None
    #clip_size = 300
    #st_map = preprocess_video_to_st_maps(video_path, output_shape, clip_size)
    #print(st_map.shape)
    #np.save("wave.npy", st_map)

    #cv2.imwrite('test.jpg', st_map[10])
    #cv2.imshow('image', st_map[10])
    #cv2.waitKey()
    ...
    #dl = DataLoaderRhythmNet(["wave.npy"], "/media/rsangers/30BE-ADAC/p1/v1/source1/wave.csv")
    #model = RhythmNet()
    #data = dl.__getitem__(0)
    #print(model(**data))
