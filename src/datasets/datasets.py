...
from ..constants import Normalization,IBIS_BOUNDS_DICT
...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.data_path,'r') as tmp_data:
            ...
            split_time = tmp_data[name]['SplitTime'][split_idx_idx] if tmp_data[name].get('SplitTime') is not None else np.array([])
...
class Dataset1D(BaseDataset):
    def __init__(self,traces_path,target,valid_data_ids,normalize,flip_signal):
        super().__init__(traces_path,target,valid_data_ids,normalize,flip_signal)
    ...
    def __getitem__(self, index):
        ...
        batch['data'] = split_data
        return batch

class DatasetCWT(BaseDataset):
    def __init__(self,cwt_data_path,target,valid_data_ids,normalize,flip_signal):
        super().__init__(cwt_data_path,target,valid_data_ids,normalize,flip_signal)
...
class Dataset1DOld(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.data_path,'r') as tmp_data:
            ...
            split_data = tmp_data[name]['SplitData'][split_idx_idx]
            split_time = tmp_data[name].get('SplitTime',[]).get(split_idx_idx,[])
